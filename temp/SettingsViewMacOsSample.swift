...
class PowerManager {
    ...
    func updateKeepOn(keepOn: Bool, hasActiveSessions: Bool) {
        let shouldPreventSleep = keepOn && hasActiveSessions
    ...
    private func createPowerAssertion() {
        ...
        // 定义电源断言的原因
        let reasonForActivity = "Prevent idle sleep due to active sessions" as CFString
        
        // 使用 kIOPMAssertionTypeNoIdleSleep 常量
        let success = IOPMAssertionCreateWithName(
            kIOPMAssertionTypeNoIdleSleep as CFString, // 强制转换为 CFString
        ...
        if success != kIOReturnSuccess {
            ...
            print("Power assertion created successfully")
        }
    }
    
    private func removePowerAssertion() {
        if assertionID != 0 {
            IOPMAssertionRelease(assertionID)
            assertionID = 0
            print("Power assertion released")
        }
    }
}
...
class Settings: ObservableObject {
    ...
    func updateKeepOn() {
        ...
        powerManager.updateKeepOn(keepOn: keepOn, hasActiveSessions: Connections.shared.sessions.count > 0)
    }
    ...
    @Published var locationTrack: Bool = false {
        didSet {
            ...
            defaults?.set(locationTrack, forKey: "locationTrack")
        }
    }
    ...
    @Published var themeName: String = "Pro" {
        didSet {
            ...
            // Update all terminal themes when global theme changes
            AppTerminalView.updateAllTerminalsTheme()
        }
    }
    ...
    func resolveFontSize(_ size: CGFloat) -> CGFloat {
        if size == 0 {
            let compact = NSScreen.main?.deviceDescription[NSDeviceDescriptionKey("NSScreenSize")] as? NSSize ?? .zero
            return NSFont.preferredFont(forTextStyle: compact.width < 480 ? .caption1 : .body).pointSize
        } else {
            return size
        }
    }
